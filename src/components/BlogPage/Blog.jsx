...
import Pagination from "../DonationPage/PaginationLogic/Pagination";
...
const Blog = () => {
  ...
  // 🔥 shuffle only once
  const [shuffledBlogs] = useState(() => shuffleArray(blogData));
  const [currentPage, setCurrentPage] = useState(1);

  const totalPages = Math.ceil(shuffledBlogs.length / CARDS_PER_PAGE);

  const startIndex = (currentPage - 1) * CARDS_PER_PAGE;
  const currentBlogs = shuffledBlogs.slice(
    startIndex,
        ...
        <div className="p-4 w-full max-w-7xl rounded-lg">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {currentBlogs.map((blog, index) => (
              <Card key={index} image={blog.image} percentage={blog.date} />
            ))}
          </div>
